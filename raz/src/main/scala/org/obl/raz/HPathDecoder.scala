...
object HPathDecoder {
  ...
  implicit def hnilHPathDecoder[PD,T, S <: PathPosition, E <: PathPosition](implicit topd:ToPathDecoder[PD,T,S,E]): HPathDecoder[PD :: HNil, T :: HNil, S, E] =
    HPathDecoder[PD :: HNil, T :: HNil, S, E] { h =>
      h1 =>
        topd(h.head).decode(h1).map( res => MatchResult(res.value :: HNil, res.rest) )
    }

  implicit def hconsHPathDecoder[H <: HList, HR <: HList, T, PD, S <: PathPosition, E <: PathPosition, S1 <: PathPosition, E1 <: PathPosition](
      implicit hr: HPathDecoder[H, HR, S1, E1],
      topd:ToPathDecoder[PD,T,S,E],
      pathAppender: PathAppender[E, S1]): HPathDecoder[PD :: H, T :: HR, S, E1] =
    HPathDecoder[PD :: H, T :: HR, S, E1] { h =>
      pth =>
        topd(h.head).decode(pth).flatMap { mr =>
