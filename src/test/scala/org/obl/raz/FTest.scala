...
class FTest {
  ...
  @Test
  def test1 = {
//		  val u1 = Raz.add("a").add(Segment.int).add("b").add(Segment.string)
    val u1 = Raz.add("a").add(Segment.int).add("b").add(Segment.string)
...
//    val qvar1 = Param.string("pippo")
    ...
    val u1 = Raz.add("a").add(Segment.int).add("b").addParam( qvar1 ).addParam("aa", "aa")
...
//     val u = Raz.add("a").add("b").add(Segment.int).addParam(Param.int("ccc")).addFragment("myfrag")
     ...
     val u1 = Raz.add("a").add("ab ba").add(Segment.int).addParam(Param.int("ccc")).addFragment("myfrag")
     ...
     val utp1 =  Raz.add("a").add("b").add(Segment.int).add(Segment.int).addFragment("f").toUriTemplate("a","b").render
     val utp1a = Raz.add("a").add("ab ba").add(Segment.int).add(Segment.int).addFragment("f").toUriTemplate("a","b").render
  ...
  @Test
  def testSgList = {
    val u1 = Raz / "base" / seq(PathConverter(Raz / "property" / Segment.string))
