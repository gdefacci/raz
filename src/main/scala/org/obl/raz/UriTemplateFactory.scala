...
object UriTemplateFactory {
  ...
  def utMatcher[T](pathf: PathF[T]): Path => Option[PathMatchResult[String, Path]] = {
    pathf.expansionKind match {
      case x if (x == ExpansionKind.stringExpansion) => PathF.pathMatcher(pathVarMatcher(stringExpansion))
      case x if (x == ExpansionKind.reservedExpansion) => PathF.pathMatcher(pathVarMatcher(reservedExpansion))
      case ExpansionKind.ParamValueStringExpansion(nm) => PathF.paramMatcher(paramVarMatcher(paramValueMatcher(nm, stringExpansion)))
      case ExpansionKind.ParamValueReservedExpansion(nm) => PathF.paramMatcher(paramVarMatcher(paramValueMatcher(nm, reservedExpansion)))
      case ExpansionKind.formStyleQueryExpansion => {
        ...
        PathF.paramMatcher(paramVarMatcher(mtchr))
      }
    }
  }
