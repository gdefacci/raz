...
object PathEncoder {
  ...
  lazy val stringSegment = PathEncoder[String](sg => Path(PathSg(sg)))
  ...
  lazy val optStringParam = PathEncoder[(String,Option[String])](sg => Path(QParamSg(sg._1, sg._2) :: Nil))
  ...
  def optStringParamValue(name:String) = PathEncoder[(Option[String])](sg => Path(QParamSg(name, sg) :: Nil))
  ...
  lazy val stringFragment = PathEncoder[String](sg => Path.empty.copy(fragment = Some(sg)))
